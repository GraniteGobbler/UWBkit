...
extern void display_distance_readout(double dist);
...
int ds_twr_responder_sts(void)
{
    ...
    while (loopCount < RANGE_COUNT)
    {
        ...
        if (!messageFlag)
        {   
            /* Call the readout when no ranging is in progress */            
            display_distance_readout(distance_array[distance_array_index-1]);
        ...
        if ((status_reg & DWT_INT_RXFCG_BIT_MASK) && (goodSts >= 0) && (dwt_readstsstatus(&stsStatus, 0) == DWT_SUCCESS))
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                else if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    // sprintf(dist_str, "%3.2f m", distance);
